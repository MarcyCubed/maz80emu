...
///
/// Returns `Done`
pub(crate) fn ld_rr_nn(state: &mut State, register: Register16, cycles: u32) -> ExecResult {
...
///
/// Doesn't return `Done`
pub(crate) fn ld_pp_r(state: &State, reg16: Register16, reg8: Register) -> ExecResult {
    ExecResult::Store {
...
///
/// Returns `Done`
pub(crate) fn ld_r_n(state: &mut State, register: Register, cycles: u32) -> ExecResult {
    state.set_register_8(register, state.z());
    ExecResult::Done(cycles)
}

/// Store the value of the 16-bit register in the immediate memory address
pub(crate) fn ld_mm_rr(state: &mut State, reg: Register16) -> ExecResult {
...
pub(crate) fn ld_mm_r(state: &State, reg: Register) -> ExecResult {
...
pub(crate) fn ld_r_r(state: &mut State, dst: Register, src: Register, cycles: u32) -> ExecResult {
    state.set_register_8(dst, state.get_register_8(src));
    ExecResult::Done(cycles)
}

/// Load the contents of a 16-bit register into another
pub(crate) fn ld_rr_rr(
    state: &mut State,
    dst: Register16,
    src: Register16,
    cycles: u32,
